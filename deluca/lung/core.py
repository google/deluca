...
class BreathWaveform(deluca.Obj):
    ...
    peep: float = deluca.field(5.0, jaxed=False)
    pip: float = deluca.field(35.0, jaxed=False)
...
def proper_time(t):
    return jax.lax.cond(t == float("inf"), lambda x: 0.0, lambda x: x, t)
...
class Controller(deluca.Agent):
    ...
    def decay(self, waveform, t):
        ...
        def false_func():
            result = jax.lax.cond(
                elapsed < waveform.xp[waveform.ex_bounds[0]],
                lambda x: 0.0,
                lambda x: 5
                * (
                    1 - jnp.exp(5 * (waveform.xp[waveform.ex_bounds[0]] - elapsed))
        ...
        result = jax.lax.cond(
            elapsed < waveform.xp[waveform.in_bounds[1]],
            lambda x: float("inf"),
            lambda x: false_func(),
            None,
        )
