...
class DelayLung(LungEnv):
    ...
    def reset(self):
        ...
        observation = Observation(predicted_pressure=0.0, time=0.0)
    ...
    def dynamics(self, state, action):
        ...
        peep = jax.lax.cond(
            state.time < self.delay, lambda x: 0.0, lambda x: state.out_history[0], None
        )

        pipe_pressure = self.inertia * state.pipe_pressure + pipe_impulse
        pressure = jnp.maximum(0, pipe_pressure - lung_pressure)

        pipe_pressure = jax.lax.cond(
            peep, lambda x: x * 0.995, lambda x: x, pipe_pressure
        )
    ...
    def __call__(self, state, action):
        u_in, u_out = action
        u_in = jax.lax.cond(
            u_in.squeeze() > 0.0, lambda x: x, lambda x: 0.0, u_in.squeeze()
        )
        ...
        state = state.replace(in_history=next_in_history, out_history=next_out_history)
