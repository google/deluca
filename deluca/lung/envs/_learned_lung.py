...
def false_func(
    ...
    state = state.replace(
        u_history=update_history(state.u_history, u_in_normalized), t_in=state.t_in + 1
    )
    ...
    default_features = jnp.hstack([jnp.zeros((default_pad_len,)), default_features])
...
class LearnedLung(LungEnv):
    ...
    def setup(self):
        ...
        default_params = self.default_model.init(
            jax.random.PRNGKey(0), jnp.ones([self.u_history_len + self.p_history_len])
        )["params"]
        ...
        boundary_params = [
            self.boundary_models[i].init(
                ...
                jnp.ones([self.u_history_len + self.p_history_len]),
            )["params"]
            for i in range(self.num_boundary_models)
        ]
    ...
    def reset(self):
        ...
        state = SimulatorState(
            ...
            p_history=jnp.hstack(
                [jnp.zeros([self.p_history_len - 1]), jnp.array([normalized_peep])]
            ),
            t_in=0,
            steps=0,
    ...
    def __call__(self, state, action):
        ...
        funcs = [
            functools.partial(self.ensemble_models[i].apply, {"params": self.params[i]})
        ...
        state = jax.lax.cond(u_out == 1, partial_true_func, partial_false_func, state)
