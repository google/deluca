...
class DeepCnn(Controller):
    ...
    def setup(self):
        ...
        if self.params is None:
            self.params = self.model.init(
                ...
                jnp.expand_dims(jnp.ones([self.history_len]), axis=(0, 2)),
            )["params"]
    ...
    def __call__(self, controller_state, obs):
        ...
        def true_func(null_arg):
            ...
            input_val = jnp.reshape(
                (trajectory @ self.featurizer), (1, self.history_len, 1)
            )
        ...
        if self.use_leaky_clamp:
            ...
            u_in = jax.lax.cond(
                u_in > self.clip, lambda x: self.clip + x * 0.01, lambda x: x, u_in
            )
        else:
