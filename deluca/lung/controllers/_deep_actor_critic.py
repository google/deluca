...
class ActorCritic(nn.Module):
    ...
    def __call__(self, x):
        ...
        logits = nn.Dense(features=self.num_actions, name="logits", dtype=self.dtype)(x)
...
class DeepAC(Controller):
    ...
    waveform: deluca.Obj = deluca.field(
        jaxed=False
    )  # moving waveform here at the cost of generality
    ...
    def setup(self, waveform=None):
        ...
        if self.params is None:
            self.params = self.model.init(
                ...
                jnp.expand_dims(jnp.ones([self.history_len]), axis=(0, 1)),
            )["params"]
    ...
    def derive_prob_and_value(self, cont_state):
        ...
        errs = jax.tree_util.tree_map(
            lambda x: jnp.expand_dims(x, axis=(1)), cont_state.errs
        )
        log_prob, value = self.model.apply(
            {"params": self.params}, (errs @ self.featurizer)
        )
