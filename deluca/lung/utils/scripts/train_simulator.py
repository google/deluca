...
def true_func(state, u_in, pressure, model):
    ...
    new_p_history = state.p_history.at[-1].set(model.p_normalizer(pressure).squeeze())
...
def predict_and_update_state(i, state_loss_model_data):
    ...
    next_state = jax.lax.cond(
        i >= model.transition_threshold, partial_true_func, partial_false_func, state
    )
    pred = model.p_normalizer(next_state.predicted_pressure)  # normalized gradient step
    return (
        next_state,
        loss + jnp.abs(model.p_normalizer(data[1, i + 1]) - pred),
        model,
        data,
    )
...
def rollout(model, data):
    ...
    loss_init = jnp.abs(
        model.p_normalizer(state.predicted_pressure) - model.p_normalizer(data[1, 0])
    )
    state_loss_model_data = (state, loss_init, model, data)

    (state, total_loss, _, _) = jax.lax.fori_loop(
        start_idx, end_idx, predict_and_update_state, state_loss_model_data
    )
...
def loop_over_loader(model_optimState_lrMult_loss, X_Y, optim, rollout_fn, scheduler):
    ...
    loss, grad = jax.value_and_grad(map_rollout_over_batch)(
        model, (X_batch, y_batch), rollout_fn
    )
...
def map_rollout_over_batch(model, data, rollout_fn):
    ...
    Returns:
      loss
    """
    rollout_partial = lambda xs: functools.partial(rollout_fn, model=model)(data=xs)
...
def train_simulator(
    ...
    for epoch in range(epochs + 1):
        ...
        X, y, prng_key = get_shuffled_and_batched_data(
            dataset, batch_size, train_key, prng_key
        )
        if epoch == 0:
            logging.info("X.shape: %s", str(X.shape))
            logging.info("y.shape: %s", str(y.shape))

        (model, optim_state, lr_mult, loss), _ = jax.lax.scan(
            loop_over_loader_partial, (model, optim_state, lr_mult, 0.0), (X, y)
        )
