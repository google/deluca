...
def loop_over_tt(
    envState_obs_ctrlState_ExpState_i, dummy_data, controller, expiratory, env, dt
):
    """loop function over time points."""
    state, obs, controller_state, expiratory_state, i = (
        envState_obs_ctrlState_ExpState_i
    )
    pressure = obs.predicted_pressure
    ...
    env.wait(max(dt - env.dt, 0))
    return (state, obs, controller_state, expiratory_state, i + 1), (
        timestamps_i,
        u_ins_i,
        u_outs_i,
        pressures_i,
...
def run_controller_scan(
    ...
    jit_loop_over_tt = jax.jit(
        functools.partial(
            loop_over_tt, controller=controller, expiratory=expiratory, env=env, dt=dt
        )
    )
    try:
