...
def rollout(controller, sim, tt, use_noise, peep, pip, loss_fn, loss):
    ...
    def loop_over_tt(ctrlState_expState_simState_obs_loss, t):
        controller_state, expiratory_state, sim_state, obs, loss = (
            ctrlState_expState_simState_obs_loss
        )
        mean = 1.5
        std = 1.0
    ...
    (_, _, _, _, loss), _ = jax.lax.scan(
        loop_over_tt, (controller_state, expiratory_state, sim_state, obs, loss), tt
    )
    return loss
...
def train_controller(
    ...
    for epoch in range(epochs):
        if pip_feed == "parallel":
            value, grad = jax.value_and_grad(rollout_parallel)(
                controller, sim, tt, use_noise, peep, jnp.array(pips), loss_fn
            )
        ...
        if pip_feed == "sequential":
            for pip in pips:
                value, grad = jax.value_and_grad(rollout)(
                    controller, sim, tt, use_noise, peep, pip, loss_fn, jnp.array(0.0)
                )
                updates, optim_state = optim.update(grad, optim_state, controller)
                controller = optax.apply_updates(controller, updates)
                per_step_loss = value / len(tt)
