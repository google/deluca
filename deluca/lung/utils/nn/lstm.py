...
class LSTM(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        lstm = LSTMNetwork(n_layers=self.n_layers, activation_fn=self.activation_fn)
        init_lstm_state = lstm.initialize_carry(batch_size, self.hidden_dim)
        init_lstm_state = (
            init_lstm_state[0].astype(jnp.float64),
...
class LSTMNetwork(nn.Module):
    ...
    def __call__(self, carry, x):
        for _ in range(self.n_layers):
            carry, x = nn.OptimizedLSTMCell(features, activation_fn=self.activation_fn)(
                carry, x
            )
        return carry, x
