...
class BreathDataset:
    ...
    def from_paths(
        ...
        splits = (np.array(splits) / np.sum(splits) * len(raw_data)).astype("int")[:-1]
        rng.shuffle(raw_data)
        obj.splits = {key: val for key, val in zip(keys, np.split(raw_data, splits))}
        ...
        for key in keys:
            obj.data[key] = (
                jnp.array([u for u, p in obj.splits[key]]),
...
def get_initial_pressure(dataset, key):
    return (
        jnp.array([dataset.p_normalizer(p[0]) for u, p in dataset.splits[key]])
        .mean()
        .item()
    )
