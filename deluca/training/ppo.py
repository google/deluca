...
def ac_rollout(
    ...
    def rollout_step(inp, counter):
        ...
        done_mask = 1.0
        return (env_next_state, env_next_obs, agent_next_state), (
            env_next_state,
            env_next_obs,
            agent_next_state,
            action,
...
def ac_parallel_rollouts(
    ...
    all_rollouts = jax.vmap(ac_rollout, (None, 0, 0, None, 0, None, None, None, None))(
        env,
        env_start_states,
        env_init_obs,
        agent,
...
@jax.jit
def ppo_loss_fn(
    agent, batch_data, clip_param: float, vf_coeff: float, entropy_coeff: float
):
...
def train_step(
    ...
    (loss, agent, optim_state), _ = jax.lax.scan(
        train_step_batch, init_carry, experiences
    )
    return loss, agent, optim_state
...
def train(env, agent, loss_func, horizon, config, workdir=None):
    ...
    key_train_agent, key_eval_agent, key_train_env, key_eval_env, key_train, key = (
    ...
    for episode in range(config.num_episodes):
        ...
        if (episode + 1) % config.episodes_per_eval == 0:
            ...
            eval_rollouts = ac_parallel_rollouts(
                env,
                eval_env_start_states,
                eval_env_init_obs,
                agent,
                eval_agent_start_states,
                horizon,
                config.gamma,
                config.lambda_,
                loss_func,
        ...
        alpha = (
            1.0 - episode / config.num_episodes
        ...
        ttt = time.time()
        experiences = ac_get_experience(
            env,
            train_env_start_states,
            train_env_init_obs,
            agent,
            train_agent_start_states,
            horizon,
            config.gamma,
            config.lambda_,
            loss_func,
        ...
        for epoch in range(config.num_epochs):
            ...
            es = jax.tree_util.tree_map(
                lambda x: jax.random.permutation(key_permute, x), experiences
            )
            ...
            es = jax.tree_util.tree_map(
                lambda x: jnp.reshape(
                    x, (num_batches, config.batch_size) + x.shape[1:]
                ),
                es,
            )
            loss, agent, optim_state = train_step(
                optim,
                optim_state,
                agent,
                es,
