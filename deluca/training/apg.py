...
Rollout = collections.namedtuple("Rollout", ["agent_states", "actions", "losses"])
...
def apg_parallel_rollouts(
    ...
    all_rollouts = jax.vmap(apg_rollout, (None, 0, 0, None, 0, None, None))(
        env,
        env_start_states,
        env_init_obs,
        agent,
...
def apg_loss(
    ...
    rollouts = apg_parallel_rollouts(
        env,
        env_start_states,
        env_init_obs,
        agent,
...
def train_chunk(
    ...
    (agent, optim_state), losses = jax.lax.scan(
        single_step, (agent, optim_state), None, length=chunk_size
    )
...
def train(env, agent, loss_func, horizon, config, workdir=None):
    ...
    for episode in range(0, config.num_episodes, config.episodes_per_eval):
        # Eval Step
        tt = time.time()
        eval_rollouts = apg_parallel_rollouts(
            env,
        ...
        # Training Step
        tt = time.time()
        agent, optim_state, losses = train_chunk(
        ...
        print(
            f"TRAINING: episode {done_eps} - score:{losses[0]} - time {time.time() - tt}"
        )
