...
def generate_histories(env: Env, agent: Agent, memory_settings: MemorySettings, N: int, T: int, rng: Array, remove_incomplete_histories: bool = True) -> Tuple[Array, Array, Array]:
    ...
    def _generate_trajectory(rng: Array) -> Tuple[Array, Array, Array]:
        ...
        def step_fn(carry: Tuple[Memory, Memory], rng: Array):
            memory, mem_agent = carry

            agent_rng, env_rng = jax.random.split(rng)
            action = agent(mem_agent.get_history(), agent_rng)

            memory = memory.resolve(action)
            mem_agent = mem_agent.resolve(action)

            # Represents complete history at time t
            obs_history, action_history = memory.get_history()

            t, state, _ = mem_agent.env_state
            ...
            mem_agent = mem_agent.prime(new_obs) 
            memory.env_state = (new_t, new_state, new_obs)
            mem_agent.env_state = (new_t, new_state, new_obs)

            new_obs_squeezed = jax.numpy.squeeze(new_obs, -1) # Remove singleton dimension

            return (memory, mem_agent), (obs_history, action_history, new_obs_squeezed)

        # Initialise memory with env.reset
        rng, env_rng = jax.random.split(rng)

        # Multiple memories: one for the histories we're generating, one for the agent who's performing actions.
        init_carry = Memory(memory_settings).reset_env(env, env_rng), Memory(agent.memory_settings).reset_env(env, env_rng)
