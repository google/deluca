...
def counterfact_loss(
    ...
    y, cost = x, 0
    for h in range(H):
        u_delta = np.tensordot(E, W[h : h + M], axes=([0, 2], [0, 1])) + off
        u = (
            U_old[t + h]
            + alpha * k[t + h]
            + K[t + h] @ (y - X_old[t + h])
...
def rolls_by_igpc(
    env_true, env_sim, U_old, k, K, X_old, D, F, alpha, w_is, ref_lr=0.1
):
    # print(ref_lr)
    ...
    W, E, off = (
        np.zeros((H + M, env_sim.state_dim)),
        np.zeros((H, env_sim.action_dim, env_sim.state_dim)),
        np.zeros(env_sim.action_dim),
    )
    ...
    for t in range(T):
        ...
        for alphaC in alpha * 1.1 * 1.1 ** (-np.arange(10) ** 2):
            ...
            if cC <= c:
                ...
                print(
                    f"(iGPC): t = {t}, r = {r}, c = {c}, alpha = {alpha}"
                )
                if log is not None:
...
class IGPCInner:
    ...
    def train(self, T, U_init=None, k=None, K=None, X=None, w_is="de", lr=None, ref_alpha=1.0):
        ...
        X, U, k, K, c = igpc_closed(self.env_true, 
                                    self.env_sim, 
                                    U_init, 
                                    T, 
                                    k,
...
class IGPC(Agent):
    ...
    def train(self, env_true, env_sim, train_steps, U_init=None, 
    		  k=None, K=None, X=None, w_is="de", lr=None, ref_alpha=1.0):
        self.env_true = env_true
        self.env_sim = env_sim
