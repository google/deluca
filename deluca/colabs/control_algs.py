...
# --- Batch-NN-DRC Controller Functions --- #

class BatchNNPolicy(nn.Module):
    d_in: int
    n: int
    d_hidden: int
    d_out: int
    ...
    @nn.compact
    def __call__(self, y_hist):
        x = y_hist.reshape((self.d_in,))
        x = nn.Dense(self.d_hidden)(x)
        x = nn.relu(x)
        x = nn.Dense(self.d_out)(x)
        return x.reshape((-1, self.n, 1))  # shape (k_plan, n, 1)


def batch_nn_drc_init_controller_state(key, init_scale, m, n, p, d, k_plan, d_hidden):
    model = BatchNNPolicy(d_in=m*p, n=n, d_hidden=d_hidden, d_out=k_plan * n)
    dummy_input = jnp.zeros((m, p, 1))
    params = model.init(key, dummy_input)
    params = jax.tree_map(lambda x: init_scale * x if 'kernel' in x.name else x, params)
    z = jnp.zeros((d, 1))
    ynat_history = jnp.zeros((2 * m, p, 1))
    controller_t = 0
    return model, params, z, ynat_history, controller_t


def batch_nn_drc_get_action(params, model, ynat_history, m):
    y_input = ynat_history[:m]
    return model.apply(params, y_input)


def batch_nn_drc_policy_loss(params, model, ynat_history, cost_fn, Q, R, m, d, p, sim, output_map, k_plan):

    def evolve_for_loss(delta, k):
        window = jax.lax.dynamic_slice(ynat_history, (k, 0, 0), (m, p, 1))
        u = batch_nn_drc_get_action(params, model, window, m)
        next_delta = sim(delta, u[0])
        return next_delta, None

    final_delta, _ = jax.lax.scan(evolve_for_loss, jnp.zeros((d, 1)), jnp.arange(m))
    
    # y_final_predicted = output_map(final_delta) + ynat_history_for_loss[-1]
    
    # u_final_for_cost = _gpc_action_for_policy_loss(m_controller, M, ynat_history_for_loss, m_controller, p)
    # cost_fn(y_final_predicted, u_final_for_cost, Q, R)

    y_input = ynat_history[:m]
    control_seq = model.apply(params, y_input)

    def rollout_step(carry, u):
        delta, total_cost = carry
        next_delta = sim(delta, u)
        y_pred = output_map(next_delta)
        cost = cost_fn(y_pred, u, Q, R)
        return (next_delta, total_cost + cost), cost

    
    (final_delta, total_cost), _ = jax.lax.scan(rollout_step, (final_delta, 0.0), control_seq)
    return total_cost


def batch_nn_drc_update_controller(
    params, z, ynat_history, controller_t,
    y_meas, u_taken,
    opt_state, optimizer, sim, output_map, loss_grad_fn
):
    # Update z for next delta
    z_next = sim(z, u_taken)
    y_nat = y_meas - output_map(z_next)

    # Update history
    ynat_history = jnp.roll(ynat_history, shift=1, axis=0)
    ynat_history = ynat_history.at[0].set(y_nat)

    grads = loss_grad_fn(params, ynat_history)
    updates, new_opt_state = optimizer.update(grads, opt_state, params)
    new_params = optax.apply_updates(params, updates)

    return new_params, z_next, ynat_history, controller_t + 1, new_opt_state


# --- Running Single Batch-NN-DRC Simulation --- #
def run_single_batch_nn_drc_trial(
    key_trial_run, A, B, C, Q_cost, R_cost, x0, sim, output_map,
    T_sim_steps, k_plan, d_hidden,
    batch_nn_drc_m, batch_nn_drc_init_scale, batch_nn_drc_lr, batch_nn_drc_lr_decay, batch_nn_drc_R_M
):
    d, n, p = A.shape[0], B.shape[1], C.shape[0]

    # Define a template for the GPC loss function that accepts the cost_eval_fn as an argument.
    def _batch_nn_drc_loss_for_grad_template(cost_eval_func_arg, model, params, ynat_hist):
        return batch_nn_drc_policy_loss(
            params, model, ynat_hist, cost_eval_func_arg, Q_cost, R_cost, batch_nn_drc_m, d, p, sim, output_map, k_plan
        )
    
    # Use functools.partial to bake in the quadratic_cost_evaluate function.
    # The gradient will be taken with respect to M (now arg 0 of final_gpc_loss_fn).
    final_batch_nn_drc_loss_fn = partial(_batch_nn_drc_loss_for_grad_template, quadratic_cost_evaluate)
    batch_nn_drc_grad_fn = jit(grad(final_batch_nn_drc_loss_fn, argnums=0))

    key_batch_nn_drc_init, key_sim_loop_main = jax.random.split(key_trial_run)
    model_controller, params_controller, z_controller, ynat_history_controller, t_controller = batch_nn_drc_init_controller_state(key_batch_nn_drc_init, batch_nn_drc_init_scale, batch_nn_drc_m, n, p, d, k_plan, d_hidden)

    current_lds_x_state = x0 # Initial LDS state
    initial_sim_time_step = 0

    optimizer = optax.sgd(learning_rate=batch_nn_drc_lr)

    init_opt_state = optimizer.init(params_controller)

    def batch_nn_drc_simulation_step(carry, key_step_specific_randomness):
        prev_lds_x, prev_params, prev_z, prev_ynat_hist, prev_t, prev_sim_t_for_disturbance, prev_opt_state = carry
        key_disturbance_for_step = key_step_specific_randomness

        u_control_t = batch_nn_drc_get_action(prev_params, model_controller, prev_ynat_hist, batch_nn_drc_m)
        ...
        next_sim_t_for_disturbance = prev_sim_t_for_disturbance + 1
        next_carry = (next_lds_x, next_params, next_z, next_ynat_hist, next_t, next_sim_t_for_disturbance, next_opt_state)
        outputs_to_collect = (cost_at_t, prev_lds_x)
        return next_carry, outputs_to_collect

    initial_carry_batch_nn_drc = (current_lds_x_state, params_controller, z_controller, ynat_history_controller, t_controller, initial_sim_time_step, init_opt_state)
    keys_for_T_scan_steps = jax.random.split(key_sim_loop_main, T_sim_steps)
    
    (_, (costs_over_time, trajectory)) = jax.lax.scan(batch_nn_drc_simulation_step, initial_carry_batch_nn_drc, keys_for_T_scan_steps)
