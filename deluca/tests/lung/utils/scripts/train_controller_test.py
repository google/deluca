...
class TrainControllerParameterizedTest(chex.TestCase):
    ...
    @parameterized.named_parameters(
        ("test1", "parallel", "multipip"), ("test2", "sequential", "singular")
    )
    ...
    def test_rollout(self, variant):
        ...
        value, _ = jax.value_and_grad(variant_rollout)(
            self.controller, self.sim, tt, use_noise, peep, pip, loss_fn, loss
        )
    ...
    def test_rollout_parallel(self, variant):
        ...
        value, _ = jax.value_and_grad(variant_rollout)(
            self.controller, self.sim, tt, use_noise, peep, jnp.array(pips), loss_fn
        )
