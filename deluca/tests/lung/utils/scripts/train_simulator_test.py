...
class TrainSimulatorParameterizedTest(chex.TestCase):
    ...
    @parameterized.named_parameters(
        ("Cosine", "Cosine"), ("ReduceLROnPlateau", "ReduceLROnPlateau")
    )
    ...
    def test_map_rollout_over_batch(self, variant):
        ...
        train_loss = variant_map_rollout_over_batch(
            self.sim, (x_train, y_train), rollout
        )
    ...
    def test_loop_over_loader(self, variant):
        ...
        (self.sim, optim_state, lr_mult, loss), _ = jax.lax.scan(
            variant_loop_over_loader, (self.sim, optim_state, lr_mult, 0.0), (x, y)
        )
