...
class TestControllerParameterizedTest(chex.TestCase):
    ...
    def setUp(self):
        ...
        self.predestined = Predestined.create(
            u_ins=jnp.zeros(
                horizon,
            )
        )
        ...
        self.residual = CompositeController(
            base_controller=self.pid, resid_controller=self.deep_cnn
        )
    ...
    @parameterized.product(
        sim_name=["learned_lung", "balloon_lung", "delay_lung", "single_comp_lung"],
        controller_name=[
            "bang_bang",
            "deep_cnn",
            "deep_mlp",
