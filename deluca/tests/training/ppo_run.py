...
flags.DEFINE_string("name", "balloon", "Name for this experiment.", short_name="n")
...
def run_reacher():
    ...
    ppo.train(reacher, agent.create(), rewards_func, horizon=10, config=get_config())
    for _ in range(10):
...
def run_learned(f):
    ...
    ppo.train(
        env=env, agent=agent, reward_fn=rewards_func, horizon=29, config=get_config()
    )
