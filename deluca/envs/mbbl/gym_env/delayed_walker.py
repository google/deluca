...
class DelayedWalker(base_env_wrapper.base_env):

    # walkers have observations from qpos and qvel
    WALKER = ['gym_dfwalker2d', 'gym_dfhopper', 'gym_dfant']

    def __init__(self, env_name='gym_dfwalker2d', rand_seed=0, misc_info=None):
        super(DelayedWalker, self).__init__(env_name, rand_seed, misc_info)
    ...
    def step(self, action):
        ...
        # )
        reward = self.reward(ob, self._old_ob, action)
    ...
    def reset(self, control_info={}):
        ...
        self.set_state(self._old_ob.copy())
        self._old_ob = self._get_observation()
    ...
    def _set_dynamics_api(self):
        ...
        def set_state(start_state):
            ...
            if self._env_name in ['gym_dfwalker2d', 'gym_dfhopper']:
                qpos[1:] = start_state[:self._len_qpos - 1]
                qvel[:] = start_state[self._len_qpos - 1:]
            elif self._env_name in ['gym_dfant']:
                qpos[2:] = start_state[:self._len_qpos - 2]
                qvel[:] = start_state[self._len_qpos - 2:]
    ...
    def _set_reward_api(self):
        ...
        def reward_derivative(state, action, target):
            num_data = len(state)
            if target == 'state':
                ...
                derivative_data[:, height_ob_pos] += - 2.0 * height_coeff * \
                    (state[:, height_ob_pos] - target_height)
...
if __name__ == '__main__':
    ...
    #                  'gym_swimmer', 'gym_ant']
    test_env_name = ['gym_dfwalker2d', 'gym_dfhopper', 'gym_dfant']

    for env_name in test_env_name:
        test_env = DelayedWalker(env_name, 1234, None)
        api_env = DelayedWalker(env_name, 1234, None)
        api_env.reset()
