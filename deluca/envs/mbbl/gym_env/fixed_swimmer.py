...
from deluca.envs.mbbl.config import init_path
from deluca.envs.mbbl import base_env_wrapper
from deluca.envs.mbbl import env_register
from deluca.envs.mbbl import env_util
from deluca.envs.mbbl.util.common import logger


class FixedSwimmer(base_env_wrapper.base_env):

    # walkers have observations from qpos and qvel
    WALKER = ['gym_fswimmer']

    def __init__(self, env_name='gym_fswimmer', rand_seed=0, misc_info=None):
        super(FixedSwimmer, self).__init__(env_name, rand_seed, misc_info)
    ...
    def _build_env(self):
        ...
        import deluca.envs.mbbl.gym_env.fix_swimmer
    ...
    def _set_dynamics_api(self):
        ...
        def set_state(start_state):
            qpos = np.zeros([self._len_qpos])
            qvel = np.zeros([self._len_qvel])

            qpos[2:] = start_state[:self._len_qpos - 2]
            qvel[:] = start_state[self._len_qpos - 2: -1]
        ...
        def fdynamics(start_state, action):
    ...
    def _set_reward_api(self):
        ...
        def reward(end_state, start_state, action):
            # the speed reward
            ...
            if self._use_pets_reward:
                reward_height = \
                    (end_state[height_ob_pos] - agent_height) / \
                    self._env.env.dt
            ...
            # the control reward
            reward_control = - ctrl_coeff * np.square(action).sum()
        ...
        def reward_tf(end_state, start_state, action):
             # the speed reward
            reward_velocity = start_state[velocity_ob_pos] if start_state is not None else tf.constant(0, dtype=tf.float32)

            # the height reward
            agent_height = start_state[height_ob_pos] if start_state is not None else tf.constant(0, dtype=tf.float32)
            ...
            # the control reward
            reward_control = - ctrl_coeff * tf.reduce_sum(tf.square(action))

            return reward_velocity + reward_height + reward_control
        self.reward_tf = reward_tf

        def reward_derivative(start_state, action, target):
            num_data = len(start_state)
            if target == 'state':
                ...
                derivative_data[:, height_ob_pos] += - 2.0 * height_coeff * \
                    (start_state[:, height_ob_pos] - target_height)
...
if __name__ == '__main__':
    ...
    #                  'gym_swimmer', 'gym_ant']
    test_env_name = ['gym_fswimmer']
    for env_name in test_env_name:
        test_env = FixedSwimmer(env_name, 1234, None)
        api_env = FixedSwimmer(env_name, 1234, None)
        api_env.reset()
        ...
        for _ in range(100):
            ...
            # test the reward api
            reward_from_api = api_env.reward(new_ob, ob, action)
            reward_error = np.sum(np.abs(reward_from_api - reward))

            # test the dynamics api
            newob_from_api = api_env.fdynamics(ob, action)
