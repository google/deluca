...
class Pets(base_env_wrapper.base_env):

    ENV = ['gym_petsCheetah', 'gym_petsReacher', 'gym_petsPusher']

    def __init__(self, env_name='gym_petsCheetah', rand_seed=0, misc_info=None):
        super(Pets, self).__init__(env_name, rand_seed, misc_info)
    ...
    def _set_reward_api(self):
        from deluca.envs.mbbl.gym_env.pets_env.reacher_config import ReacherConfigModule
        from deluca.envs.mbbl.gym_env.pets_env.pusher_config import PusherConfigModule
        from deluca.envs.mbbl.gym_env.pets_env.half_cheetah_config import HalfCheetahConfigModule
        ...
        def reward(start_state, action, next_state=None):
            ...
            reward = -(cfg_module.ac_cost_fn(action[None, :]) +
            ...
            if target == 'state':
                ...
                if self._env_name == 'gym_petsCheetah':
                    ...
                    derivative_data[:, 14: 17] = \
                        np.sign(start_state[:, 14: 17] -
                                start_state[:, 17: 20]) * 0.5

                    # obj_goal_dist
                    derivative_data[:, -3:] = \
                        np.sign(start_state[:, -3:] -
                                start_state[:, 17: 20]) * 1.25

                    derivative_data[:, 17: 20] = \
                        np.sign(start_state[:, 17: 20] -
                                start_state[:, 14: 17]) * 0.5 + \
                        np.sign(start_state[:, 17: 20] -
                                start_state[:, -3:]) * 1.25
...
if __name__ == '__main__':
    ...
    for env_name in test_env_name:
        print(env_name)
        test_env = Pets(env_name, 1234, {})
        api_env = Pets(env_name, 1234, {})
        api_env.reset()
